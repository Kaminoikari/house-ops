...
function writeDailyReport(scan, newReports) {
  const reportsData = newReports.map(f => extractReportScore(f)).sort((a, b) => (b.score ?? 0) - (a.score ?? 0));
  ...
  if (scan.newItems.length) {
    const byDist = {};
    for (const l of scan.newItems) byDist[l.district] = (byDist[l.district] || 0) + 1;
    lines.push('## 📊 各區新增');
    lines.push('');
    for (const [d, n] of Object.entries(byDist).sort((a, b) => b[1] - a[1])) lines.push(`- ${d}：${n} 筆`);
    lines.push('');
  }
...
async function main() {
  const scan = runScan();
  const newReports = runEvalOnNewItems(scan.newItems);
  writeDailyReport(scan, newReports);

  console.error(`\n━━━━━━━━━━━━━━━━━━━━━━━━`);
  console.error(`Daily Run — ${TODAY} 完成`);
  console.error(`━━━━━━━━━━━━━━━━━━━━━━━━`);
  console.error(`新物件：${scan.newItems.length} 筆 → 產 ${newReports.length} 份報告`);
  console.error(`價格變動：${scan.priceChanged.length} 筆`);
  console.error(`已下架：${scan.expired.length} 筆`);
  console.error(`日報：reports/daily/${TODAY}.md`);
}
