...
```bash
cd /Users/win.yu/Github/house-ops
git init
```
...
```bash
git add .
git commit -m "chore: initialize house-ops repo structure"
```
...
```yaml
# Canonical tracker statuses for house-ops
...
1. **What is house-ops** — one paragraph
...
```bash
git add -A
git commit -m "feat: complete house-ops v0.1.0 — all modes and scripts integrated"
```
