...
本教學依三種使用者類型分別說明完整工作流程。找到自己的情境，照著走就對了。
...
| 類型 | 說明 | 主要用到的模式 |
|------|------|--------------|
| 🏠 **租屋族** | 找租屋，無自有房產 | scan、rent、compare、visit |
| 🔑 **首購族** | 人生第一次買房 | afford、scan、buy、compare、visit |
| 🔄 **換屋族** | 已有房產，計劃賣舊買新 | switch、afford、scan、buy、compare、visit |
...
flowchart TD
    ...
    Decision -->|繼續追蹤| Offer[出價 → 談判 → 簽約 → 完成]
...
不管哪種類型，**第一次開啟 Claude Code 時**，系統會自動偵測缺少設定檔，啟動初始設定流程。
...
flowchart TD
    ...
    DB -->|有命中| SK5(["跳過：命中排除條件"])
    DB -->|無| Qualified(["第一階段通過 ✅"])

    Qualified --> P2["第二階段：完整評估\nrent.md 或 buy.md"]
    P2 --> Extract[資料萃取\n價格 · 坪數 · 格局 · 設備]
...
與租屋族相同流程，使用 `compare`、`prepare visit`、看屋後記錄。
...
- 所有個人設定（`config/profile.yml`、`modes/_profile.md`）**系統更新永遠不會覆寫**。
